...
export default function NotificationsPopover() {
  ...
  // dont know how the notifications will be shaped yet so any array for now
  const [notifications, setNotifications] = useState<any[]>([])

  const totalUnRead = notifications.length > 0 ? notifications.filter((item) => 'isUnRead' in item && item.isUnRead).length : 0;
  ...
  return (
    <>
      ...
      <MenuPopover open={openPopover} onClose={handleClosePopover} sx={{ width: 360, p: 0, mt: 4.3 }}>
        ...
        <Scrollbar sx={{ height: { xs: 340, sm: 'auto' } }}>
          {/* To be reconfigured when we have the notifications pulling in */}
          {/* <List
            disablePadding
            subheader={
